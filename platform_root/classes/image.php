<?php
	...
	class image {
		...
		/**
		 *  Copy an image from tmp to destination
		...
		/**
		 *  Rename an image
		 *  Args: (str) old image file name, (str) new image file name
		 *  Return: (boolean) success
		 */
		public function renameImage($destination, $name = false) {
			if ($this->imageFile) {
				if (!$name) {
					$name = $this->imageFile['name'];
				}
				...
				if ($this->imageFile['tmp_name'] != $destination.$name) {
					if (rename($this->imageFile['tmp_name'], $destination.$name)) {
						$this->__construct($name, 'file', $destination);
						return $this->exists();
					}
				}
			}
